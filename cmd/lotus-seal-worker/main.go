...
var c2RunCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		sectorSize := abi.SectorSize(cctx.Int64("size"))
		rootPath := cctx.String("path")
		sid, err := storiface.ParseSectorID(cctx.String("sector"))

		writeResult := func(sector abi.SectorID, res string) error {
			outPath := filepath.Join(rootPath, storiface.FTCache.String(), storiface.SectorName(sector), "c2_res")
			ioutil.WriteFile(outPath, []byte(res), os.ModePerm)
			return nil
		}

		err = writeResult(sid, "0")
		if err != nil {
			return err
		}
		...
		writeC2Out := func(sector abi.SectorID, out storage.Proof) error {
			outPath := filepath.Join(rootPath, storiface.FTCache.String(), storiface.SectorName(sector), "c2_out")
		...
		readC1Out := func(sector abi.SectorID) (storage.Commit1Out, error) {
			outPath := filepath.Join(rootPath, storiface.FTCache.String(), storiface.SectorName(sector), "c1_out")
		...
		cids, err := sb.SealCommit2(ctx, ref, pc1o)
		if err != nil {
			return xerrors.Errorf("SealCommit2: %w", err)
		}

		err = writeC2Out(sid, cids)
		if err != nil {
			return err
		}

		err = writeResult(sid, "1")
		if err != nil {
			return err
		}
