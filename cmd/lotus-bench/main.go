...
var proveCmd = &cli.Command{
	...
	Action: func(c *cli.Context) error {
		return proveInner(c, false)
	},
}

// usedInParallel = true if the func was called from p2c2 cli command
// in this case input.json is second argument
func proveInner(c *cli.Context, usedInParallel bool) error {
	if c.Bool("no-gpu") {
		err := os.Setenv("BELLMAN_NO_GPU", "1")
		if err != nil {
			return xerrors.Errorf("setting no-gpu flag: %w", err)
	...
	if !usedInParallel {
		...
		if c.Args().Len() < 2 {
			return xerrors.Errorf("Usage: lotus-bench p2c2 [p2input.json] [c2input.json]")
		}
		inb, err = ioutil.ReadFile(c.Args().Get(1))
	}
...
var calibrateCmd = &cli.Command{
	Name:      "calibrate",
	Usage:     "Calibrate a constants for a SealCommit2",
	ArgsUsage: "[input.json]",
	Flags: []cli.Flag{
		&cli.BoolFlag{
			Name:  "no-gpu",
	...
	Action: func(c *cli.Context) error {
		...
		_, err = sb.CalibrateSealCommit2(context.TODO(), ref, c2in.Phase1Out)
		if err != nil {
			return err
		}
