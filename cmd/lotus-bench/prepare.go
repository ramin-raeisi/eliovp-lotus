...
var prepareBenchCmd = &cli.Command{
	Name:      "prepare",
	Usage:     "Seal pre-commit 1 phase",
	...
	Action: func(c *cli.Context) error {
		return p1Inner(c, true)
	},
}

func p1Inner(c *cli.Context, isPrepare bool) error {
	...
	if !c.Args().Present() {
		return xerrors.Errorf("Usage: lotus-bench prepare [output.json]")
	}
	...
	sealTimings, pc1o, err := runP1(sb, sbfs, sectorNumber, parCfg, mid, sectorSize, []byte(c.String("ticket-preimage")))
	if err != nil {
		return xerrors.Errorf("failed to run P1: %w", err)
	}

	pc1o.SectorBuilderPath = sbdir

	b, err := json.Marshal(&pc1o)
	if err != nil {
		return err
	}

	if err := ioutil.WriteFile(p1file, b, 0664); err != nil {
		log.Warnf("%+v", err)
	}
...
func runP1(sb *ffiwrapper.Sealer, sbfs *basicfs.Provider, numSectors int, par ParCfg, mid abi.ActorID, sectorSize abi.SectorSize, ticketPreimage []byte) ([]SealingResult, *PrepareOut, error) {
