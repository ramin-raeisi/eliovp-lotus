...
func (sb *Sealer) AddPiece(ctx context.Context, sector storage.SectorRef, existingPieceSizes []abi.UnpaddedPieceSize, pieceSize abi.UnpaddedPieceSize, file storage.Data) (abi.PieceInfo, error) {
	...
	io.TeeReader(io.LimitReader(file, int64(pieceSize)), pw)
	...
	level := bits.TrailingZeros64(uint64(pieceSize.Padded())) - 2 - 5
	c, err := commcid.PieceCommitmentV1ToCID(zerocomm.PieceComms[level][:])
	if err != nil {
		return abi.PieceInfo{}, err
	}
...
func (sb *Sealer) SealGenerateLabelsBench(ctx context.Context, sector storage.SectorRef, ticket abi.SealRandomness, pieces []abi.PieceInfo) (out storage.PreCommit1Out, err error) {
	paths, done, err := sb.sectors.AcquireSector(ctx, sector, storiface.FTUnsealed, storiface.FTSealed|storiface.FTCache, storiface.PathSealing)
	if err != nil {
		return nil, xerrors.Errorf("acquiring sector paths: %w", err)
	}
	...
	// TODO: context cancellation respect
	p1o, err := ffi.SealGenerateLabelsBench(
		sector.ProofType,
		paths.Cache,
		paths.Unsealed,
		paths.Sealed,
...
func (sb *Sealer) CalibrateSealCommit2(ctx context.Context, sector storage.SectorRef, phase1Out storage.Commit1Out) (storage.Proof, error) {
	return ffi.CalibrateSealCommitPhase2(phase1Out, sector.ID.Number, sector.ID.Miner)
...
>>>>>>> 556e97af17bad9d41f48ac9bbfaac56e94d9aae7
}
