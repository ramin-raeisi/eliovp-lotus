...
func (m *Sealing) handleAddPiece(ctx statemachine.Context, sector SectorInfo) error {
	...
	for i, piece := range pending {
		...
		proposal, err := m.Api.StateMarketStorageDealProposal(ctx.Context(), deal.deal.DealID, tok)
		...
		log.Infow("deal added to a sector", "deal", deal.deal.DealID, "sector", sector.SectorNumber, "piece", proposal.PieceCID)
		...
		res.NewPieces = append(res.NewPieces, Piece{
			Piece:    abi.PieceInfo{Size: deal.size.Padded(), PieceCID: proposal.PieceCID},
			DealInfo: &deal.deal,
		})
	}
